...
function updateAfterResult(userId, wasWin, actual, betPlaced) {
    ...
    state.predictionOutcomes.push(isSkip ? "LOSS" : outcome);
    ...
    if (typeof autobetState !== 'undefined' && autobetState[userId]) {
        ...
        } else if (betPlaced && !wasWin) {
            st.consecutiveLoss++;
            st.level++;
            if (st.level > (cfg && cfg.maxLvl ? cfg.maxLvl : 15)) {
                st.level = 1;
                st.consecutiveLoss = 0;
            }
...
async function runPredict(userId, chatId) {
    ...
    if (signal.skip) {
        ...
        await send(chatId, `⏭️ Skip Round (${signal.reason})`);
