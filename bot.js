...
async function captchaLogin(userId, chatId, phone, password, bot, logBoth) {
    ...
    try {
        ...
        console.log('[LOGIN] Navigating to 13L WinGo 30S page to trigger GetBalance request...');
        console.log('[LOGIN] Navigating directly to 13L WinGo 30S page via URL...');
...
async function fetchList() {
    try {
        ...
        // Attached platform returns { data: { list: [...] } }.
        const rawList = Array.isArray(payload?.data?.list)
            ? payload.data.list
            : Array.isArray(payload?.data)
                ? payload.data
                : null;
        if (!rawList || rawList.length === 0) {
            console.error("[FETCH LIST ERROR] Platform response did not contain data.list");
            return null;
        }
        return rawList.map(item => ({
            issueNumber: String(item?.issueNumber ?? item?.issue_number ?? item?.period ?? item?.id ?? ""),
            number: Number.parseInt(item?.number ?? item?.winNumber ?? item?.result_number ?? item?.num ?? item?.value ?? -1, 10),
...
async function placeBet(userId, chatId, period, prediction, predType, level, amountOverride) {
    ...
    if (!token || token.length < 20) {
        const failure = { ok: false, msg: "Token இல்லை. முதலில் Login press பண்ணு." };
        await send(chatId, "❌ " + failure.msg);
        return failure;
    }
    ...
    if (!token || token.length < 20) {
        const failure = { ok: false, msg: "Token missing before bet request." };
        await send(chatId, "❌ " + failure.msg);
        return failure;
    }
    ...
    if (predType === "NUMBER") bc = "Number_" + String(prediction);
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            const d = r.data || {};
            const apiMessage = String(d.msg ?? d.message ?? d.msgCode ?? d.data?.msg ?? "");
            const codeValue = String(d.code ?? d.msgCode ?? d.data?.code ?? "").toLowerCase();
            const successMessage = /^(succeed|success|successful|ok)$/i.test(apiMessage.trim());
            const betAccepted = codeValue === "0" || successMessage;
            console.log(`[BET RESP] http:${r.status} code:${d.code ?? d.msgCode ?? d.data?.code} msg:${apiMessage} accepted:${betAccepted}`);
            ...
            const failure = { ok: false, msg: apiMessage || JSON.stringify(d).slice(0, 120) };
            await send(chatId, "❌ Bet fail: " + failure.msg);
            return failure;
            ...
            const failure = { ok: false, msg: "Network error during bet: " + err.message };
            await send(chatId, "❌ " + failure.msg);
            return failure;
        }
    }
    ...
    return { ok: false, msg: "All bet retries exhausted" };
}
// ============================================================
// ============================================================
